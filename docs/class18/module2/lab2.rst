...
2. Download and install Docker CE
   (https://docs.docker.com/engine/installation/)
3. Build the sample docs ``./containthedocs-build.sh``. The first time
   you build a container (~1G in size) will be downloaded from Docker
   Hub.
4. Open the ``docs/_build/html/index.html`` file on you system in a web
   browser
...
6. Open the ``docs/_build/html/index.html`` file on you system in a web
   browser
...
.. WARNING:: While the document has been converted to rST format you
will still need to refactor the rST to use the structure implemented in
this template.
...
- ``containthedocs-build.sh``: Build HTML docs using
  ``make -C docs html`` to ``docs/_build/html``
- ``containthedocs-clean.sh``: Clean the build directory using
  ``make -C docs clean``
- ``containthedocs-cleanbuild.sh``: Clean the build directory and build
  HTML docs using ``make -C docs clean html``
- ``containthedocs-convert.sh``: Convert a Word ``.docx`` file to rST

.. |Issues| image:: https://img.shields.io/github/issues/f5devcentral/f5-agility-labs-nginx.svg
   :target: https://github.com/f5devcentral/f5-agility-labs-nginx/issues
.. |Build the Docs| image:: https://github.com/f5devcentral/f5-agility-labs-nginx/actions/workflows/build-the-docs.yml/badge.svg
   :target: https://github.com/f5devcentral/f5-agility-labs-nginx/actions/workflows/build-the-docs.yml
